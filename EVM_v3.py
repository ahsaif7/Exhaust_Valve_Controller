#Compatible with 12v DC External Supply
...
# ----------------------
# ACS712 parameters (20A version)
# ----------------------
sensitivity = 0.100   # V/A (ACS712-20A)
samples_calibration = 50
samples_smooth = 10

# Thresholds tuned for 12V exhaust valve motor
# Adjust if your motor's stall current differs
forward_limit = 1.2     # A (stall current when opening)
backward_limit = -1.2   # A (stall current when closing)
...
def calibrate_offset():
    ...
    return (avg / 4095) * 3.3   # offset voltage in V

def read_current():
    raw = current_pin.read()
    voltage = (raw / 4095) * 3.3
    current = (voltage - ACSoffset) / sensitivity
    return -current  # sign flipped for wiring
...
while True:
    ...
    if last_button_state == 1 and button_state == 0:  # button press detected
        ...
        while True:
            ...
            if direction == 'open' and current > forward_limit:
                ...
                # Recalibrate at open position
                print("Recalibrating ACS712 offset at open position...")
                sleep(1)
                ACSoffset = calibrate_offset()
                print("New offset:", round(ACSoffset, 3), "V")
